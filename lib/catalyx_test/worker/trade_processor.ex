defmodule CatalyxTest.TradeProcessor do
  ...
  defp process_group(period, {market_symbol, data}) do
    ...
    {transaction, new_sma_values, new_sma_count, trend, opening_at, opening, closing_at, closing,
     highest,
     lowest} =
      Enum.reduce(
        data,
        {Ecto.Multi.new(), [], 0, trend, opening_at, opening, closing_at, closing, highest,
         lowest},
        fn
          trade,
          {transaction, sma_values, sma_count, trend, opening_at, opening, closing_at, closing,
           highest, lowest} ->
            %Trade{
    ...
    period_record =
      Finances.change_candle_indicator(period_record, %{
        trend: trend,
        opening_at: opening_at,
        opening_price: opening,
        closing_at: closing_at,
    ...
    |> Ecto.Multi.insert_or_update(
      "upsert_period_record_#{inspect(period)}_#{market_symbol}",
      period_record
    )
